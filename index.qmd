...
```{python}
import pandas as pd
csvFile = pd.read_csv('choc_chip_cookie_ingredients.csv')
```
...
1.  Which Ingredient has the highest average rating?
2.  Does the amount of recipes an ingredient is included in effect its rating?
3.  Does quantity play a significant factor in an ingredient's rating?
4.  How many total recipes are in this data set?
5.  Which ingredient is seen the most?
...
There are a lot of rows (1010) that have an NA value in the rating column. Also, the mean rating of the recipies is 0.815.
...
```{r}
less_twenty <- proportion[proportion$count < 20, "Ingredient"]
less_twenty <-unlist(less_twenty)
cat(less_twenty, sep = ", ")
```
...
```{r}
library(ggplot2)
ggplot(cookies) + geom_point(aes(x = X, y = Rating))
```
...
From this graph, you can look for which ingredient in a recipe (variable X on the x-axis) has the highest rating (y-axis). From there, you can search for trends, like if one ingredient is consistently receiving a high/low rating. You could then create the best possible cookie recipe.
