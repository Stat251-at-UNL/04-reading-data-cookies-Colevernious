...
result <- paste(less_twenty, collapse = ", ")
print(result)
```
...
```{r}
library(ggplot2)
ggplot(cookies) + geom_point(aes(x = Ingredient, y = Rating))
```
...
While the y-axis is pretty straight forward, it may be smart to change or bound the x-axis so the Ingredients that go along it can be visibly seen. If the x-axis is visible, you can then look at which ingredient tends to have higher ratings when used.
